---
name: architecture
description: Core philosophy, architectural rules, domain model, and constraints for this accounting engine. Use when discussing system design, adding new domains, reviewing architectural decisions, or explaining how the system works.
---
...
3. **Strict Precision.** Monetary values are processed via the `shopspring/decimal` library. Never use `float64` for money.
...
type Proposal struct {
    DocumentTypeCode    string         // REQUIRED: 'JE', 'SI', 'PI', 'SO', 'GR', or 'GI'
...
- **Clarification Loop**: If the AI returns `IsClarificationRequest: true`, the REPL asks the user the AI's question, appends the answer to the conversation context, and re-submits.
- **Routing**: `/` prefix → deterministic command. No prefix → AI agent.
...
5. **Normalize + Validate**: Go validates balance in base currency.
6. **Review (REPL only)**: User sees proposal and approves or rejects.
7. **Commit**: `Ledger.Commit()` runs everything in a single atomic DB transaction.

## 7. Future Roadmap

1. **Phase 6**: Reporting — materialized views, `/pl` (P&L) and `/bs` (Balance Sheet).
2. **Phase 4**: Policy & Rule Engine — configurable account mapping registry.
3. **Phases 5, 7, 8**: Approvals, AI Expansion, External Integrations — deferred.
